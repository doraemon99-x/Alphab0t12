...
zeksApikey = 'Alya chan bot WhatsApp' //ganti pake apikey lu biar limitnya gk cepet abis 
...
case 'setexif':
		    ...
		    case 'colong':
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `Radhin`
...
break
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, ©Radhin Al haady & Alya putri zayyan```')
					fs.unlinkSync(ran)
					})
					break
...
*Chapter Akhir* : ${komik.chapter_akhir}`
			...
			case 'term':
					...
					if (!q) return fakegroup(mess.wrongFormat)
					exec(q, (err, stdout) => {
					if (err) return fakegroup(`Alya chan bot WhatsApp:~ ${err}`)
					if (stdout) {
					fakegroup(stdout)
					}
					})
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by Radhin X Alya 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
break
 ...
 members_ids = []
					...
					ini_list.push({
					"displayName": 'Owner Alphabot',
					"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${alpha.user.name}`}\nORG: Radhin Al haady & Alya putri zayyan;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
					})
					}
					hehe = await alpha.sendMessage(from, {
					"displayName": `${ini_list.length} kontak`,
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "Radhin Al haady & Alya putri zayyan", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Alya chan bot WhatsApp :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
